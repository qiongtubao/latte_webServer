			...
			(function() {
				...
				this.run = function(callback) {
					...
					serverDomain.on("error", function(err) {
						if(self.debug) {
				 			...
				 			var filename = "./logs/otherError/"+Date.now()+".log";
							latte_lib.fs.writeFile(filename, (err.stack? err.stack.toString(): err.toString())  );	
				 		}
					});
				...
				this.onRequest = function() {
					...
					return function( req, res ) {
						var urlObject = url.parse(decodeURIComponent(decodeURIComponent(req.url)));
