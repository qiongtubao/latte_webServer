(function(define) { 'use strict';
	define("latte_webServer/post/index", ["require", "exports", "module", "window"], 
	function(require, exports, module, window) {		
		(function() {
			var latte_lib = require("latte_lib");
			var EventEmitter = latte_lib.events;
			var MultipartParser = require("./multipartParser").MultipartParser
				, QuerystringParser = require("./querystringParser").QuerystringParser
				, Stream = require('stream').Stream
				, OctetParser = require("./octetParser").OctetParser
			...
			latte_lib.inherits(Post, EventEmitter);
			(function() {
				this.pause = function() {
					var req = this.req;
					if(!req) {
				...
				this._parseContentType = function() {
					if (this.bytesExpected === 0) {
  					...
  					var self = this;
  					switch(Headers.getContentType(this.req)) {
  						case "octet-stream":
  						...
  						case "json":
  							...
  							break;
  						case "multipart":
  							var m = self.req.headers['content-type'].match(/boundary=(?:"([^"]+)"|([^;]+))/i);
							if(m) {
								self._initMultipart(m[1] || m[2]);
							} else {
								self.onError(new Error("bad content-type headers, no multipart boundary"));
							}
  							break;
  						case "text":
  							self._initTextcoded();
  						...
  						break;
  						case null:
  							this.onError(new Error("bad content-type header, no content-type"));
  							break;
  						default:
  							//dummyParser(this);
				...
				this._initTextcoded = function() {
					var self = this;
					var parser = new OctetParser();
					var data = "";
				...
				this.write = function(buffer) {
					if (this.error) {
					  ...
					  console.log("data:",buffer, bytesParsed == buffer.length);
					...
					if(part.filename === undefined) {
						...
						part.on("data", function(buffer) {
							...
							if(self._fieldsSize > self.maxFieldsSize) {
								self.onError(new Error("maxFieldsSize exceeded, received"+self._fieldsSize + "bytes of field data"));
								return;
							}
