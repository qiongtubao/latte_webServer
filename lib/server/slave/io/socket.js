		...
		(function() {
			this.onOpen = function () {
			  ...
			  this.sendPacket('open', JSON.stringify({
			    ...
			    , pingInterval: this.server.config.pingInterval
			    , pingTimeout: this.server.config.pingTimeout
			  }));
			...
			this.setPingTimeout = function () {
			  ...
			  }, self.server.config.pingInterval + self.server.config.pingTimeout);
			};
			...
			this.maybeUpgrade = function (transport) {
			  ...
			  }, this.server.config.upgradeTimeout);
			...
			this.getAvailableUpgrades = function () {
			  ...
			  for (var i = 0, l = allUpgrades.length; i < l; ++i) {
			    ...
			    if (this.server.config.transports.indexOf(upg) != -1) {
